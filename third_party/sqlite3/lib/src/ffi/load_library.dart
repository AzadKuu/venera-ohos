...
DynamicLibrary _defaultOpen() {
  if (Platform.isAndroid) {
    ...
    // HarmonyOS (ohos) 没有系统级 libsqlite3.so，且 sqlite3 包的平台枚举
    // 不包含 ohos。加载随 HAP 打包的 libsqlite3.so（由鸿蒙 NDK 交叉编译）。
    // 该分支确保后台 isolate（Isolate.run / compute）也能正常加载，
    // 因为 overrideForAll 只在调用它的 isolate 内生效。
    return DynamicLibrary.open('libsqlite3.so');
